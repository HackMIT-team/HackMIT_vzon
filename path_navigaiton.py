...
img= cv2.imread("A_black_image.jpg")
traj = cv2.resize(img, (500, 500)) 
...
while(cap.isOpened()):
    ...
    final_point = cv2.circle(traj, (300,140), 1, list((0,255,0)),6 )
    x,y = 250,250
    for i in estimated_path:
        try:
            if abs(np.round(cur_pose[0, 3],2) - x) > 1:
                x = np.round(cur_pose[0, 3],2)
            if abs(np.round(cur_pose[2, 3],2) - y) > 1:
                y = np.round(cur_pose[2, 3],2)
            result = cv2.circle(traj, (int(250+x*3), int(400+ y*3)), 1, list((0, 0, 255)), 6)
            cv2.imshow('trajectory', traj)
            if int(250+x*3)>= 300:
                print(int(250+x*3),"turn Left")
            elif int(400+ y*3) <= 140:
                print(int(400+ y*3),"turn right")
